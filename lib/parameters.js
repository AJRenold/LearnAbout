...
getParametersPlaylists = function (terms) {
  ...
  deepExtend(true, baseParameters, {options: {sort: {_id: -1}}})
...
getParameters = function (terms) {
  ...
  switch (terms.view) {
    ...
    case 'topic': // same as top for now
      var parameters = deepExtend(true, baseParameters, {options: {sort: {sticky: -1, score: -1}}});
      break;
  ...
  if(terms.postDifficulty != '' && !terms.category && terms.subCategories.length < 1)
    _.extend(parameters.find, { 'difficulty': terms.postDifficulty });

  if(typeof terms.category != 'undefined' && !!terms.category){
    var ands = [{'categories.slug': terms.category }];
    if(terms.postDifficulty !== ''){
      ands.push({ 'difficulty': terms.postDifficulty });
    }
    _.extend(parameters.find, { $and : ands });
  }

  if(typeof terms.subCategories != 'undefined' && terms.subCategories.length > 1) {
    console.log(terms.postDifficulty)
    var ands = _.map(terms.subCategories, function(cat){
      return { 'categories.name': cat }
    });
    if(terms.postDifficulty !== ''){
      ands.push({ 'difficulty': terms.postDifficulty })
    }
